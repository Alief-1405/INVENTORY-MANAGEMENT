...
import { 
  getSalesOrderDetailsByNo, 
  confirmDispatch, 
  cancelOrderByWarehouse,
  getDispatchQueue
...
export default function DispatchOrderPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<"BELUM_DO" | "SUDAH_DO" | "BATAL">("BELUM_DO");
  ...
  // 2. Fetch dispatch queue dynamically based on status and search query
  const { data: queueRes, isLoading: loadingQueue } = useQuery({
    queryKey: ["dispatch-queue", statusFilter, soSearchInput],
    queryFn: () => getDispatchQueue(statusFilter, soSearchInput),
    enabled: isGudangOrAdmin
  });
  ...
  const handleSelectSO = async (soNumber: string) => {
    setSoSearchInput(soNumber);
    setSearching(true);
    setSoDetails(null);
    setShowCancelConfirm(false);
    try {
      const res = await getSalesOrderDetailsByNo(soNumber);
      if (res.success && res.data) {
        setSoDetails(res.data);
        toast.success(`Data Sales Order ${soNumber} berhasil dimuat.`);
      } else {
        toast.error(res.message || "Sales Order tidak ditemukan.");
      }
    } catch (err) {
  ...
  const handleConfirmDispatch = async () => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        // Invalidate queries to refresh the queue table and stats
  ...
  if (loadingProfile) {
    return (
      <div className="flex h-[60vh] flex-col items-center justify-center gap-3">
        <Loader2 className="h-10 w-10 animate-spin text-teal-655" />
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      {/* Grid Atas: Panel Pencarian & Detail SO */}
      <div className="grid gap-6 md:grid-cols-3">
        
        {/* Panel Pencarian */}
        <Card className="md:col-span-1 border border-white/40 bg-white/60 dark:bg-zinc-900/60 backdrop-blur-md rounded-2xl shadow-sm h-fit">
          <CardHeader className="pb-3">
            <CardTitle className="text-base font-bold text-[#0B132B] dark:text-zinc-150 flex items-center gap-2">
              <Search className="h-4.5 w-4.5 text-indigo-600" />
              Cari & Filter Sales Order (SO)
            </CardTitle>
            <CardDescription className="text-slate-500 text-xs">
              Pilih status di dropdown untuk memfilter antrean di bawah, atau masukkan nomor SO lengkap untuk pencarian cepat.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSearch} className="flex flex-col gap-3">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-550 dark:text-zinc-400 uppercase">Status Pesanan</label>
                <Select
                  value={statusFilter}
                  onValueChange={(val: any) => setStatusFilter(val)}
                  disabled={!isGudangOrAdmin}
                >
                  <SelectTrigger className="w-full rounded-xl text-xs font-bold border-slate-200/80 dark:border-zinc-800 bg-white dark:bg-zinc-950">
                    <SelectValue placeholder="Pilih Status" />
                  </SelectTrigger>
                  <SelectContent className="rounded-xl border border-slate-250 dark:border-zinc-800 bg-white dark:bg-zinc-950">
                    <SelectItem value="BELUM_DO" className="text-xs font-semibold cursor-pointer">Belum DO (Antrean)</SelectItem>
                    <SelectItem value="SUDAH_DO" className="text-xs font-semibold cursor-pointer">Sudah DO (Selesai)</SelectItem>
                    <SelectItem value="BATAL" className="text-xs font-semibold cursor-pointer">Batal (Cancelled)</SelectItem>
                  </SelectContent>
                </Select>
              </div>

              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-550 dark:text-zinc-400 uppercase">Nomor SO</label>
                <input
                  type="text"
                  value={soSearchInput}
                  ...
                  disabled={!isGudangOrAdmin}
                  className="w-full py-2.5 px-3 bg-white border border-slate-200/80 dark:bg-zinc-950 dark:border-zinc-800 rounded-xl text-xs font-mono font-bold text-slate-800 dark:text-zinc-200 outline-none uppercase placeholder:lowercase"
                />
              </div>

              <Button
                type="submit"
                disabled={searching || !isGudangOrAdmin || !soSearchInput.trim()}
                className="w-full bg-[#0B132B] hover:bg-[#1C2541] active:bg-[#0B132B] dark:bg-zinc-850 dark:hover:bg-zinc-800 text-white rounded-xl text-xs font-bold shadow-md cursor-pointer flex items-center justify-center gap-1.5 h-10"
              >
                {searching ? (
        ...
        <div className="md:col-span-2">
          {soDetails ? (
              ...
              <Card className="md:col-span-2 border border-white/40 bg-white/60 dark:bg-zinc-900/60 backdrop-blur-md rounded-2xl shadow-sm">
                <CardHeader className="border-b border-slate-100 dark:border-zinc-850 pb-3">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    <div>
                      <CardTitle className="text-lg font-bold text-[#0B132B] dark:text-zinc-150 flex items-center gap-1.5">
                        <FileText className="h-5 w-5 text-indigo-650" />
                        Rincian Order: {soDetails.soNumber}
                      </CardTitle>
                      <CardDescription className="text-slate-550 dark:text-zinc-400 text-xs mt-0.5">
                    ...
                    <div>
                      {soDetails.status === "DONE" ? (
                        ...
                        <span className="inline-flex items-center gap-1 rounded-full bg-red-50 text-red-800 border border-red-250 px-2.5 py-0.5 text-[10px] font-extrabold uppercase dark:bg-red-950/20 dark:text-red-400">
                          <Clock className="h-3.5 w-3.5" />
                          Menunggu Pembayaran
                        </span>
                      )}
                ...
                <CardContent className="pt-5 space-y-6">
                  ...
                  <div className="grid gap-4 sm:grid-cols-3 text-xs">
                    <div className="p-3 bg-white/40 dark:bg-zinc-950/20 rounded-xl border border-slate-100/50 dark:border-zinc-850 flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="font-extrabold text-slate-800 dark:text-zinc-200">
                          {soDetails.status === "DONE" || soDetails.status === "PAID" ? (
                            <span className="text-emerald-600 font-extrabold">LUNAS ({soDetails.paymentMethod || "QRIS"})</span>
                          ) : (
            ...
            <Card className="border border-white/40 bg-white/60 dark:bg-zinc-900/60 backdrop-blur-md rounded-2xl shadow-sm h-full flex flex-col items-center justify-center p-8 text-center text-slate-400 min-h-[200px]">
              <Package className="h-10 w-10 mb-3 stroke-1 text-indigo-500" />
              <span className="text-xs font-bold text-slate-800 dark:text-zinc-200">Belum Ada Rincian Sales Order Dipilih</span>
              <span className="text-[10px] text-slate-500 mt-1 max-w-xs">
                Silakan pilih salah satu pesanan pada tabel antrean di bawah, atau masukkan nomor SO di form pencarian untuk mulai proses.
              </span>
            </Card>
          )}
        </div>
      ...
      {/* Tabel Antrean SO (Bagian Bawah) */}
      <Card className="border border-white/40 bg-white/60 dark:bg-zinc-900/60 backdrop-blur-md rounded-2xl shadow-sm">
        <CardHeader className="pb-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <CardTitle className="text-base font-bold text-[#0B132B] dark:text-zinc-150 flex items-center gap-1.5">
              <Clock className="h-4.5 w-4.5 text-indigo-650" />
              Daftar Antrean & Histori Sales Order
            </CardTitle>
            <CardDescription className="text-slate-500 text-xs">
              {statusFilter === "BELUM_DO" && "Menampilkan antrean pesanan masuk yang belum diproses pengeluarannya (FIFO)."}
              {statusFilter === "SUDAH_DO" && "Menampilkan riwayat pesanan yang sudah dibuatkan surat jalan (Delivery Order)."}
              {statusFilter === "BATAL" && "Menampilkan daftar pesanan yang dibatalkan."}
            </CardDescription>
          </div>
          <Badge className="bg-indigo-50 text-indigo-700 dark:bg-indigo-950/20 dark:text-indigo-400 font-bold border-indigo-200 w-fit">
            {queue.length} Pesanan
          </Badge>
        </CardHeader>
        <CardContent>
          {loadingQueue ? (
            <div className="flex h-32 items-center justify-center gap-2">
              ...
              <span className="text-xs text-slate-500 font-semibold">Memuat data antrean...</span>
            </div>
          ) : queue.length === 0 ? (
            <div className="flex h-32 flex-col items-center justify-center text-slate-400 py-6 border border-dashed border-slate-200 dark:border-zinc-800 rounded-xl bg-slate-50/20">
              <Package className="h-8 w-8 mb-2 stroke-1 text-slate-350" />
              <span className="text-xs font-bold">Tidak ada data Sales Order</span>
              <span className="text-[10px] text-slate-400">Sesuaikan filter status atau pencarian Anda.</span>
            </div>
          ) : (
            <div className="overflow-hidden rounded-xl border border-slate-100 dark:border-zinc-800 bg-white/35">
              <Table>
                <TableHeader className="bg-[#FAF9F5]/70 text-[#0B132B] dark:bg-zinc-950 dark:text-slate-350 font-bold border-b border-slate-100 dark:border-zinc-800 uppercase tracking-wider text-[10px]">
                  <TableRow>
                    <TableHead className="font-bold px-4 py-3">No. SO</TableHead>
                    <TableHead className="font-bold px-4 py-3">Tanggal Transaksi</TableHead>
                    <TableHead className="font-bold px-4 py-3">Nama Pelanggan</TableHead>
                    <TableHead className="font-bold px-4 py-3">Item Barang</TableHead>
                    <TableHead className="font-bold px-4 py-3 text-center">Status</TableHead>
                    <TableHead className="font-bold px-4 py-3 text-center">Aksi</TableHead>
                  </TableRow>
                </TableHeader>
                  ...
                  {queue.map((item: any) => {
                    return (
                      <TableRow key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-zinc-900/30 transition-colors">
                        <TableCell className="font-mono font-bold text-slate-800 dark:text-zinc-200 px-4 py-3">
                          {item.soNumber}
                        </TableCell>
                        <TableCell className="text-slate-500 font-medium px-4 py-3">
                          {new Date(item.createdAt).toLocaleString("id-ID", {
                            dateStyle: "medium",
                            timeStyle: "short",
                          })}
                        </TableCell>
                        <TableCell className="font-bold text-slate-800 dark:text-zinc-200 px-4 py-3">
                          {item.customerName}
                        </TableCell>
                        ...
                        <TableCell className="text-center px-4 py-3">
                          {item.status === "DONE" ? (
                            <Badge className="bg-emerald-50 hover:bg-emerald-50 text-emerald-800 dark:bg-emerald-950/20 dark:text-emerald-400 border-emerald-250 font-bold uppercase text-[9px]">
                              Sudah DO
                            </Badge>
                          ) : item.status === "CANCELLED" ? (
                            <Badge className="bg-rose-50 hover:bg-rose-50 text-rose-800 dark:bg-rose-950/20 dark:text-rose-400 border-rose-250 font-bold uppercase text-[9px]">
                              Batal
                            </Badge>
                          ) : item.status === "PAID" ? (
                            <Badge className="bg-amber-50 hover:bg-amber-50 text-amber-850 dark:bg-amber-950/20 dark:text-amber-400 border-amber-250 font-bold uppercase text-[9px]">
                              Belum DO (Lunas)
                            </Badge>
                          ) : (
                            <Badge className="bg-amber-50 hover:bg-amber-50 text-amber-850 dark:bg-amber-950/20 dark:text-amber-400 border-amber-250 font-bold uppercase text-[9px]">
                        ...
                        <TableCell className="text-center px-4 py-3">
                          <Button
                            onClick={() => handleSelectSO(item.soNumber)}
                            size="sm"
                            disabled={!isGudangOrAdmin}
                            className="bg-[#0B132B] hover:bg-[#1C2541] active:bg-[#0B132B] dark:bg-zinc-800 dark:hover:bg-zinc-700 text-white rounded-lg font-bold text-[10px] px-2.5 py-1.5 flex items-center gap-1 mx-auto cursor-pointer shadow-sm disabled:opacity-55 disabled:cursor-not-allowed"
                          >
                            Proses DO
